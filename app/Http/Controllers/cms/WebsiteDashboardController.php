...
class WebsiteDashboardController extends Controller
{
    public function index()
    {
        ...
        $courses                    =    Course::select('id','name','display_order','topic_id', 'reference')->orderBy('display_order', 'desc')->orderBy('topic_id')->get();
        $data['total_courses']      =    $courses->count();
        $data['total_enquiries']    =    Enquiry::count();
       
        foreach ($courses as $course) {
            $course_schedule['course_data']    = $course;
            $course_schedule['schedule_count'] = $course->count();
